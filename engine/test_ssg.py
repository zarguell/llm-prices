...
def _providers():
    return {
        ...
        "ogo": {"name": "OGO", "models": {
            ...
            "whisper-mini": {"name": "Whisper Mini",
...
def test_flatten_extracts_pricing_and_capabilities():
    ...
    # audio-in model derives its type; chat models get text flags
    assert by_key["ogo/whisper-mini"]["mtype"] == "transcription"
    assert by_key["ogo/whisper-mini"]["text_in"] is False
    assert by_key["zai/glm-flash"]["text_in"] and by_key["zai/glm-flash"]["text_out"]
...
def test_stats_coverage_best_value_and_medians():
    ...
    assert stats["providers_n"] == 2 and stats["models_n"] == 6
    assert stats["priced_n"] == 6 and stats["coverage"] == 100.0
    # zero-priced models never rank: free-tier (0/0) and half-free (0 in)
    assert stats["zero_priced_n"] == 2
    # whisper-mini (audio in -> text out) is transcription, not a chat model:
    # despite being the cheapest positive rate (0.01) it must not rank
    assert stats["cheapest"][0]["key"] == "zai/glm-flash"
    assert all(r["input"] > 0 for r in stats["cheapest"])
    assert all(r["mtype"] in ("chat", "chat multimodal", "omni")
               for r in stats["cheapest"])
    mix = {m["mtype"]: m for m in stats["modality_mix"]}
    assert mix["transcription"]["n"] == 1
    assert mix["chat"]["n"] == 5  # glm-flash, glm-big, mimo + 2 zero-priced
    ...
    # medians exclude zero rates: zai [0.1, 5.0] -> 2.55
    ogo = next(p for p in stats["providers"] if p["provider"] == "ogo")
    zai = next(p for p in stats["providers"] if p["provider"] == "zai")
    # ogo median keeps positive non-chat rates: [0.14, 0.01] -> ~0.075
    assert abs(ogo["median_input"] - 0.075) < 1e-9
